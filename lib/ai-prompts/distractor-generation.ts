...
export function buildDistractorGenerationPrompt(
  ...
  if (contextWord) {
    ...
    promptText += `
    Choose the three most relevant distractor words from the following list that could plausibly be mistaken for the correct answer. Do not use the correct answer itself as a distractor.`;
  }

  promptText += ` 
    IMPORTANT: Under no circumstances should the word "${correctAnswer}" ${contextWord ? `or "${contextWord}" ` : ""}be included in the distractors.
    If no words from the list are relevant, you can generate appropriate distractors.
